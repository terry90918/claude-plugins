...
41. [ ] Codex 屬帳號層級設定，是否自動審查依各貢獻者個人帳號、repo 不覆寫，不主動 request 或等待其回應；合併前確認 CI 全綠、`mergeable=MERGEABLE`、`mergeStateStatus` 為 `CLEAN`／`UNSTABLE`／`HAS_HOOKS`、CodeRabbit 無未處理意見；不設定自動 Claude PR review pipeline
