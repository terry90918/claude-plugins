...
| Release 判定 | 一支 event-aware `is-release-commit.sh` 同時辨識 push merge／squash 與官方 release PR | release 判定必須是單一、可測試的 predicate；所有 gate 與 deploy 呼叫它決定要不要跳過。`github-release`／`release-pr` 這兩個 release cut 步驟本身不呼叫它——它們的職責就是每次觸發都跑，有沒有東西可 cut 交給 release 工具自己判斷。**release-only 的 side effect（例如 npm publish）呼叫同一個 predicate，但 predicate 只證明「這是 release commit」，不證明「這次執行真的剛 cut 出新版」**——重跑或補跑同一個 release commit 時 predicate 一樣為真。要讓這類 side effect 在重跑下安全，還需要額外的冪等檢查（例如發布前查 registry／version 是否已存在），不能只靠 predicate。 | 精確 branch、body marker 與提交格式依 provider／release tool 設定調整，但只比 branch 名稱不足。 |
| Production deploy | deploy 等待全部必要 gate；target registry 持有 workspace、UUID、health URL；依 dependency graph 選擇 target | deploy 必須等全部必要 gate 綠燈才觸發，無法判定是否受影響時 fail open 為部署，不可靜默漏部署——這三條是通用的。**target 怎麼定義則因規模而異**：本規範給一般 repo 的預設是 UUID／API 呼叫直接寫在 `.drone.yml`（見 `ci-workflow-templates.md`），每個 app 各一個 deploy step；`entire` 自己因為要管理 6 個 target，才把它們收斂進 `coolify-deploy.ts` 的 target registry，pipeline 只負責觸發。**有 commit-aware health endpoint 時**，每次部署必須等終態並讀回 commit。 | 沒有多 app、Coolify 或 commit-aware health endpoint 時（例如標準 fire-and-forget 模板），以「部署 API 呼叫成功」作為對應的可觀察驗收。target registry 只在 app 數量或部署邏輯複雜到單一 YAML 難以維護時才考慮採用，不是預設。 |
